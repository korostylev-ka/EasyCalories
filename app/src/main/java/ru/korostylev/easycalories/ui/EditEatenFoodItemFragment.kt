...
import ru.korostylev.easycalories.databinding.FragmentEditEatenFoodItemBinding
...
class EditEatenFoodItemFragment : Fragment() {
    private var _binding: FragmentEditEatenFoodItemBinding? = null
    private val binding: FragmentEditEatenFoodItemBinding
        get() = _binding ?: throw RuntimeException("FragmentEditEatenFoodItemBinding is null")
    ...
    override fun onCreateView(
        ...
        _binding = FragmentEditEatenFoodItemBinding.inflate(layoutInflater)
        return binding.root
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        addPortionWeightWatcher()
        bindViews()
        setClickListeners()
    }
    ...
    private fun setClickListeners() {
        ...
        binding.date.setOnClickListener {
            val datePickerDialog = DatePickerDialog(requireContext(), DatePickerDialog.OnDateSetListener { view, year, monthOfYear, dayOfMonth ->
                ...
                binding.date.setText(getSelectedStringDate())
            }, year, month, day)
        ...
        binding.clockIcon.setOnClickListener {
            val timePickerDialog = TimePickerDialog(requireContext(), TimePickerDialog.OnTimeSetListener { view, hourOfDay, minute ->
                ...
                binding.time.setText(getSelectedStringTime())
            }, hour, minute, true)
            timePickerDialog.show()
        }
        binding.buttonSave.setOnClickListener {
            val proteins = binding.proteinsValue.text.toString()
            val proteinsFloat = proteins.toFloat()
            val fats = binding.fatsValue.text.toString()
            val fatsFloat = fats.toFloat()
            val carbs = binding.carbsValue.text.toString()
            val carbsFloat = carbs.toFloat()
            val calories = binding.caloriesValue.text.toString()
            val caloriesFloat = calories.toFloat()
            val portionWeight = binding.portionWeightValue.text.toString()
    ...
    private fun addPortionWeightWatcher() {
        ...
        binding.portionWeightValue.addTextChangedListener(portionWeightValueWatcher)
    }
